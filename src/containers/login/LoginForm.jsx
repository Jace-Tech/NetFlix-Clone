...
const LoginForm = () => {
    ...
    const [loginCredientials, setLoginCredentials] = useState({ email: "", password: "" })
        ...
        <Box flex={1} as={"section"}>
            <Container w={{ base: "100%", md: "95%" }} maxW={"container.sm"} display={"flex"} justifyContent={"center"}>
                <Box w={{ base: "100%", sm: "90%", lg: "80%" }} color={"white"} bgColor={"rgba(0, 0, 0, .75)"} p={{ base: 10, sm: 12, lg: 16 }} borderRadius={"md"}>
                    ...
                    <Box mt={10}>
                        <LoginInput 
                            handleChange={(e) => setLoginCredentials(prev => ({...prev, email: e.target.value}))}
                            value={loginCredientials.email}
                            placeholder={"Email or phone number "}
                        />
                        <LoginInput 
                            handleChange={(e) => setLoginCredentials(prev => ({...prev, password: e.target.value}))}
                            type={"password"}
                            value={loginCredientials.password}
                            placeholder={"Enter your password"}
                        />
                    </Box>

                    <Box mt={10}>
                        <CustomButton text={"Sign In"} hover style={{ w: "100%", py: 3, fontWeight: "semibold" }} />
                        <HStack mt={3}>
